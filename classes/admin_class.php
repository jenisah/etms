...
public function change_password_for_employee($data) {
    ...
    try {
        $update_password = $this->db->prepare("UPDATE tbl_admin SET password = :password, temp_password = NULL WHERE user_id = :user_id");
        ...
        $_SESSION['update_user_pass'] = 'Password changed successfully';

        $stmt = $this->db->prepare("SELECT user_role FROM tbl_admin WHERE user_id = :user_id");
...
public function update_user_password($data, $id, $user_role) {
    ...
    try {
        ...
        $_SESSION['update_user_pass'] = 'Password updated successfully';

        // Redirect based on user role
        if ($user_role == 1) {
            header('Location: admin-manage-user.php');
        } else {
            header('Location: user-profile.php'); // Assuming there's a user-profile.php for non-admin users
        }
        exit(); // Ensure the script stops executing after the header
    } catch (PDOException $e) {
        echo $e->getMessage();
    }
}
...
public function manage_all_info($sql, $params = []) {
    try {
        $stmt = $this->db->prepare($sql);
        $stmt->execute($params);
        return $stmt;
